<footer class="main-footer dark p-0">
    <div class="sub-footer">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-4 mt-3 mb-2">
                    <div class="text-left">
                        <h6 style="color: white;">Construyendo el futuro de Baja California Sur.</h6>
                    </div>
                </div>
                <div class="col-md-4 mt-3 mb-2 text-center">
                    <img style="width: 5rem;" src="/img/logo2.png" alt="">
                </div>
                <div class="col-md-4 mt-3 mb-2">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="text-right">
                                <h6 style="color: white;">&#174;NALMIA <span id="demo"></span></h6>

                            </div>
                        </div>
                        <div class="col-md-12" style="margin-top: 1.5rem;">
                            <div class="text-right">
                                <a href="mailto:contact@nalmia.com" style="text-decoration: underline;">
                                    <i class="fa fa-envelope"></i> contact@nalmia.com
                                </a>
                            </div>
                        </div>
                    </div>
                </div>

                    <div class="col-md-12 text-center" style="margin-top: 1.5rem;">
                        <h6>Todos los derechos reservados. Desarrollado por <a target="_blank"
                                href="https://bananagroup.mx">Banana Group Marketing</a>.</h6>
                    </div>
                </div>
            </div>
        </div>
</footer>